...
def create_calendar(service, calendar_name: str) -> str:
    ...
    try:
        acl_rule = {
            ...
            "role": "owner"  # Выдаём высокие привилегии
        }
...
def get_or_create_calendar(service, calendar_name: str) -> str:
    ...
    if not calendar_id:
        logging.info(f"Создаём новый календарь '{calendar_name}'.")
        ...
        if prev_sched_path.exists():
            ...
            logging.info("Удалён previous_schedule.json для нового календаря.")
    else:
...
def remove_calendar(service, calendar_id: str):
    ...
    try:
        ...
        logging.info(f"Календарь {calendar_id} удалён.")
    except Exception as e:
